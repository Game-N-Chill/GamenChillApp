...
Result Help::run(MKTG::Generator &gen, std::vector<std::string> &args)
{
    ...
    std::cout << std::left << "-- " << std::setw(10) << "load"     << std::setw(50) << "<path>"                    << "Charge le fichier excel du bracket" << std::endl;
    ...
    std::cout << std::left << "-- " << std::setw(10) << "dir"      << std::setw(50) << "<path>"                    << "Change le dossier où les rendus seront créés" << std::endl;
