...
void Winner::Player::parseSkins()
{
    ...
    if (!std::filesystem::is_directory(pathDir)) {
        Error() << "ERROR: " << pathDir << " is not a directory, can't setup skins" << GGL::end;
        return;
    }
...
{
    flux << "Name: " << this->_name << GGL::end;
    flux << "Character: " << this->_character << GGL::end;
    flux << "Skin: " << this->getSkin() << GGL::end;
}
