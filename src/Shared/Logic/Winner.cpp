...
void loadWebFile(std::string url)
{
    ...
    try {
        str = req.Get(url);
    } catch (const GGL::Exception::CurlPerform &e) {
        Error() << e.what() << GGL::end;
        return;
    } catch (const GGL::Exception::CurlResponseCode &e) {
        Error() << e.what() << GGL::end;
        return;
    } catch (const GGL::Exception &e) {
        Error() << e.what() << GGL::end;
        return;
    }
    data = json::parse(str);
    ...
    for (auto &it : data["participants"]) {
        ...
        if (rank >= 1 && rank <= 8) {
            ...
            if (std::regex_match(name, matches, pattern)) {
                ...
                Error() << "WARNING: can't match regex for " << name << GGL::end;
            }
        }
    }
}
...
{
    if (path.empty()) {
        Error() << "ERROR: path empty" << GGL::end;
        return;
    }
    ...
    try {
        ...
        Error() << "WARNING: title format is incorrect (" << e.what() << ")" << GGL::end;
    }
    ...
    try {
        ...
        Error() << "WARNING: subtitle format is incorrect (" << e.what() << ")" << GGL::end;
    }
    ...
    try {
        ...
        if (date.empty()) {
            dataWinner->setDate( GGL::getTimeFormat("%d/%m/%Y"));
        } else {
            dataWinner->setDate(date);
        }
    } catch (const OpenXLSX::XLValueTypeError &e) {
        Error() << "WARNING: date format is incorrect (" << e.what() << ")" << GGL::end;
    }
...
static void createWinnerShared(std::string path, const std::array<Data::Winner::Team<N>, PLAYER_GRAPH_COUNT> &array)
{
    if (path.empty()) {
        RESET_PROGRESS_TASK;
        Error() << "can't open file: " << path << GGL::end;
        return;
    }
    ...
    canva.draw();
    DO_PROGRESS_TASK;
    canva.save(dataWinner->getOutputDir(),  GGL::getTimeFormat("%Y_%m_%d__%H_%M_%S"));
    DO_PROGRESS_TASK;
}
